...
class Pager {
	...
	*/
    public function set_link ( $link, $is_shop = TRUE )
    {
        ...
        //静态页面处理
        //$arr_link = explode('list-', $link);
        //if(sizeof($arr_link)==1){
        //    $arr_link = explode('tags-', $link);
        //}
    ...
    public function get_next_page_btn ()
    {
        if ( $this->_page < $this->_total_page )
        {
			...
			if($this->_static){
                $s_page = $this->_page + 1;
                $s_link = '/list-'.$this->_cid.'-'.$s_page.'.html';
